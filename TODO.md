...
- [ ] DB-консоли: выполнение (feedback 19.09, критично). `rider_execute_console`:
  выполнить стейтмент открытой консоли в её контексте, вернуть CSV +
  статистику; плюс чтение гридов последнего выполнения. Рецепт не покрывает —
  новый тул оправдан (см. CONTRACT.md). Research: API выполнения JdbcConsole,
  диапазоны стейтментов, доступ к результатам
